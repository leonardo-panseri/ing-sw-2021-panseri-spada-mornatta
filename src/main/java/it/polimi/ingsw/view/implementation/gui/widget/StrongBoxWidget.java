...
public class StrongBoxWidget extends FlowPane {
    ...
    public void updateStrongBox(Resource resType, int numberOf) {
        ...
        Platform.runLater(() -> {
            ...
            switch (resType) {
                ...
                case COIN -> {
                    ((Label) ((VBox) coinRow.getChildren().get(1)).getChildren().get(0)).setText("" + numberOf);
                }
                case SERVANT -> {
                    ((Label) ((VBox) servantRow.getChildren().get(1)).getChildren().get(0)).setText("" + numberOf);
                }
                case STONE -> {
                    ((Label) ((VBox) stoneRow.getChildren().get(1)).getChildren().get(0)).setText("" + numberOf);
                }
                case SHIELD -> {
                    ((Label) ((VBox) shieldRow.getChildren().get(1)).getChildren().get(0)).setText("" + numberOf);
                }
            }
    ...
    public void removeStrongBoxRow(Resource resource) {
        ...
        switch (resource) {
            ...
            case COIN -> {
                this.getChildren().remove(coinRow);
            }
            case SERVANT -> {
                this.getChildren().remove(servantRow);
            }
            case STONE -> {
                this.getChildren().remove(stoneRow);
            }
            case SHIELD -> {
                this.getChildren().remove(shieldRow);
            }
